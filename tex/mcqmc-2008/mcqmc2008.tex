...
call.  Both of them have been widely known as a techeneque of speed
...
a floating point number, hence the convertion is slow.
...
this method is fast is that implicit conversion uses signed interger
which size is larger than unsigned one.
...
standard and also available in 2008 starndard.  Figure~\ref{fig:ieee}
...
ajusted so that the fraction part has a number $f (1 \le f < 2)$, and
$e$ has an offset 1023, then the total of the format shows the number
$f \times 2^{e - 1023}$. And the first 1 of $f$ is ommited in the
...
\item 
  ...
  $p_1$ is a parameter $12 \le \textrm{sl1} < 64$.
...
`AND' respectively, every array is declared to have two unsigned 64-bit
integers and \texttt{w0} is for $w_0$, and so on.

Fig.~\ref{fig:dsfmt} shows the recursion in circuit like diagram.
\begin{figure}
  \begin{center}
...
considered as an automaton, with the state space $S=\bbf2^{b_u + b_w
  \times N}$ where $b_u$ is size of vector $u$ and $b_w$ is that of
...
Let $p$ be the Mersenne exponent, and $N:=\lceil p/b_w \rceil$.  Then,
...
Let $V_w$ is this subspace, and $V_wr = V_w \cup V_r$.
In very high probability, restriction of $V_r$ to the subspace
has dimention $r$.
...
\]
This means $b(t)\phi_r = 1 (\mod \phi_p)$ hence we get the projection
$P_p = b(f)\phi_r(f).$

We concern the affine function $f'_p$. If $\exist z\, f_p(z) = z$,
...
$(1 - t)$ is coprime with $\phi_p$, then we can get $(1 - t)^{-1} \mod 
\phi_p$ using extended Euclidean algolithm, and we can get fix point
by assigning $f_p$ into $t$ and applying $c_p.$ Table~\ref{tab:pcv}
shows such fix points corespond with mersenne exponents.
...
\]
then $b(t)$ is $(t - 1)^{-1} \mod \phi_p(t).$ Now we get
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{rrrrrrr} \hline
      MEXP & $N$ & $M$ & SL1 & MASK(LOW) & MASK(HIGH) & weight \\ \hline \hline
      521 & 5 & 3 & 25 & \texttt{0x000fbfefff77efff} 
      & \texttt{0x000ffeebfbdfbfdf} & 273 \\
      1279 & 13 & 9 & 19 & \texttt{0x000efff7ffddffee} 
      & \texttt{0x000fbffffff77fff} & 673 \\
      2203 & 21 & 7 & 19 & \texttt{0x000fdffff5edbfff} 
      & \texttt{0x000f77fffffffbfe} & 1076 \\
      4253 & 41 & 19 & 19 & \texttt{0x0007b7fffef5feff} 
      & \texttt{0x000ffdffeffefbfc} & 2233 \\
      11213 & 108 & 37 & 19 & \texttt{0x000ffffffdf7fffd} 
      & \texttt{0x000dfffffff6bfff} & 5684 \\
      19937 & 192 & 117 & 19 & \texttt{0x000ffafffffffb3f} 
      & \texttt{0x000ffdfffc90fffd} & 9756 \\ \hline
    \end{tabular}
  \end{center}
\end{table}
...
\begin{remark}
  ...
  the amount of bit-mixing.  The column \textbf{weight} in Table~\ref{params}
  shows the number. In the case of dSFMT(1993), the number of
  nonzero terms is 9756, which is much larger than 135 of MT, and
  larger than 8585 of WELL19937c \cite{WELL}.
\end{remark}
...
\begin{table}
  \begin{center}
    \caption{Period certification vectors}
    \label{tab:pcv}
    \begin{tabular}{c|rr} \hline
      MEXP & \multicolumn{1}{c}{Fix Point} 
      & \multicolumn{1}{c}{Period Certification} \\ \hline \hline
...
Table~\ref{tab:dd} lists the dimension defects $d(v)$ of SFMT19937 (as
a 32-bit integer generator) and of MT19937, for $v=1,2,\ldots, 32$.
SFMT has smaller values of the defect $d(v)$ at 26 values of $v$. The
converse holds for 6 values of $v$, but the difference is small.  The
total dimension defect $\Delta$ of SFMT19937 as a 32-bit integer
generator is $4188$, which is smaller than the total dimension defect
$6750$ of MT19937.
...
\begin{table}
  \begin{center}
    \begin{tabular}{|r|rrrrrr||r|rrrrrr|} \hline
      ...
      \multicolumn{8}{|l|}{total demension defect $\Delta$} 
