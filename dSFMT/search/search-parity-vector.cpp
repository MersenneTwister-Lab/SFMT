...
void set_bit(in_status *st, GF2X& f, int *bit_pos) {
	...
	if (*bit_pos < maxdegree -128) {
	    k = *bit_pos % 52;
	    j = (*bit_pos / 52) % 2;
	    i = *bit_pos / (52 * 2);
	} else {
	    k = (maxdegree - *bit_pos) % 64;
	    j = (maxdegree - *bit_pos) / 64;
	    i = N;
	}
...
void search_lung (GF2X& f) {
    static in_status bases[LUNG_WIDTH];
    int i, j;
    int count;
    int bit_pos = 0;
    int size = 2;
    int base_num = (WORD_WIDTH - (mexp - 128) % WORD_WIDTH);
...
void search_parity_check_vector(in_status base[], int size) {
    ...
    mx.SetDims(LUNG_WIDTH, size);
    for (i = 0; i < LUNG_WIDTH; i++) {
	for (j = 0; j < size; j++) {
