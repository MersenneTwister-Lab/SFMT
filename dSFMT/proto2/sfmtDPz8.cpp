...
inline static void do_recursion(uint64_t a[2], uint64_t b[2],
				uint64_t lung[2]) {
    uint64_t t0, t1, L0, L1, p0, p1;

    t0 = a[0];
    t1 = a[1];
    L0 = lung[0];
    L1 = lung[1];
    p0 = (t0 << DSFMT::sl1) ^ (L1 >> 32) ^ (L1 << 32) ^ (b[0] & DSFMT::msk1);
    p1 = (t1 << DSFMT::sl1) ^ (L0 >> 32) ^ (L0 << 32) ^ (b[1] & DSFMT::msk2);
    a[0] = (p0 >> 12) ^ t0;
    a[1] = (p1 >> 12) ^ t1;
    lung[0] = p0 ^ (p0 << DSFMT::sl2);
    lung[1] = p1 ^ (p1 << DSFMT::sl2);
}
