...
static uint64_t MSK3 = 7;
static uint64_t MSK4 = 7;
...
void setup_param(unsigned int p1, unsigned int p2, unsigned int p3, 
    ...
    POS1 = p1 % (N - 2) + 1;
    SL1 = (p2 & 0xffff) % (32 - 1) + 1;
    SL2 = (p13 % 4) * 2 + 1; 
    SR1 = (p2 >> 16) % (32 - 1) + 1;
    sr_mask = (ff << 32) | (ff >> (32 + SR1));
    MSK1 = ((uint64_t)(p4 | p5 | p6) << 32) | (p8 | p9 | p10);
    MSK2 = ((uint64_t)(p6 | p7 | p8) << 32) | (p10 | p11 | p12);
    MSK1 = MSK1 & sr_mask;
    MSK2 = MSK2 & sr_mask;
    MSK3 = (ff << (SL1 + 32)) | (ff >> 32);
    SR2 = (p3 & 0xffff) % (32 - 1) + 1;
    MSK4 = (ff << 32) | (ff >> (32 + SR2));
}
...
void print_param(FILE *fp) {
    ...
    fprintf(fp, "MSK3 = %016llx\n", MSK3);
    fprintf(fp, "MSK4 = %016llx\n", MSK4);
    fflush(fp);
}
...
static inline void do_recursion(uint64_t a[2], uint64_t b[2],
    ...
    lshift128(x, a, SL2);
    r0 = a[0] ^ x[0] ^ ((b[0] >> SR1) & MSK1) 
	^ ((c[0] << SL1) & MSK3) ^ ((c[0] >> SR2) & MSK4) ^ lung[1];
    r1 = a[1] ^ x[1] ^ ((b[1] >> SR1) & MSK2) 
	^ ((c[1] << SL1) & MSK3) ^ ((c[0] >> SR2) & MSK4) ^ lung[0];
