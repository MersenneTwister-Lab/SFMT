...
void fill_state_random(sfmt_t *sfmt, FILE *frandom) {
    ...
    sfmt->sfmt[(p / 4) % NN][p % 4] = w;
    sfmt->sfmt[NN - 1][0] = (uint32_t)getw(frandom);
    sfmt->sfmt[NN - 1][1] = (uint32_t)getw(frandom);
    sfmt->sfmt[NN - 1][2] = (uint32_t)getw(frandom);
    sfmt->sfmt[NN - 1][3] = (uint32_t)getw(frandom);
}
...
void test_shortest(char *filename) {
	...
	if (lcmcount > 100000) {
	    printf("failure deg = %d < %d\n", (int)deg(lcmpoly), maxdegree);
	    printf("weight = %d\n", (int)weight(lcmpoly));
	    return;
	}
	errno = 0;
