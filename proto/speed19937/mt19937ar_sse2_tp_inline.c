...
INLINE static void mm_temper_line(uint32_t *x, 
				  __m128i and1, __m128i and2) {
    __m128i a0, a1;
...
INLINE static void gen_rand_array(uint32_t array[], uint32_t blocks)
{
    __m128i r, u_mask, l_mask, mat_a, one, and1, and2;
    int i, j;
	...
	_mm_store_si128((__m128i *)&array[i], r);
	mm_temper_line(&array[i - N], and1, and2);
    }
    for (j = 0; i < N * blocks; i += 4, j += 4) {
	r = mm_recursion(&array[i - N], &array[i + 1 - N], &array[i + M - N],
			 u_mask, l_mask, mat_a, one);
	_mm_store_si128((__m128i *)&array[i], r);
