...
INLINE void gen_rand_all(void) {
    ...
    const vector unsigned int sl3 = (vector unsigned int)(SL3, SL3, SL3, SL3);
    const vector unsigned int sr1 = (vector unsigned int)(SR1, SR1, SR1, SR1);
    const vector unsigned int mask = (vector unsigned int)(MSK1, MSK2, MSK3, MSK4);
    ...
    r2 = sfmt[N - 1];
    for (i = 0; i < N - POS1; i++) {
	r = vec_recursion(sfmt[i], sfmt[i + POS1], r1, r2, sl1, sl3, sr1,
			  mask, perm, perm_sl);
	sfmt[i] = r;
	r1 = r2;
	r2 = r;
    ...
    for (; i < N; i++) {
	r = vec_recursion(sfmt[i], sfmt[i + POS1 - N], r1, r2, sl1, sl3, sr1,
			  mask, perm, perm_sl);
	sfmt[i] = r;
	r1 = r2;
	r2 = r;
...
INLINE static void gen_rand_array(vector unsigned int array[], uint32_t blocks)
{
    ...
    for (; i < N; i++) {
	r = vec_recursion(sfmt[i], array[i + POS1 - N], r1, r2, sl1, sl3, sr1,
			  mask, perm, perm_sl);
	array[i] = r;
	r1 = r2;
	r2 = r;
