...
__attribute__((always_inline)) 
    ...
    lshift128(&x, a, SL2);
    r->a[0] = a->a[0] ^ x.a[0] ^ ((b->a[0] >> SR1) & MSK1) ^ c->a[0] 
	^ (c->a[0] << SL1) ^ d->a[3];
    r->a[1] = a->a[1] ^ x.a[1] ^ ((b->a[1] >> SR1) & MSK2) ^ c->a[1]
	^ (c->a[1] << SL1) ^ d->a[2];
    r->a[2] = a->a[2] ^ x.a[2] ^ ((b->a[2] >> SR1) & MSK3) ^ c->a[2]
	^ (c->a[2] << SL1) ^ d->a[0];
    r->a[3] = a->a[3] ^ x.a[3] ^ ((b->a[3] >> SR1) & MSK4) ^ c->a[3]
	^ (c->a[3] << SL1) ^ d->a[1];
}
