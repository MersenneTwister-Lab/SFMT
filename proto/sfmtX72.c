...
static inline void do_recursion(uint32_t a[4], uint32_t b[4],
    ...
    a[0] = a[0] ^ x[0] ^ y[0] ^ (b[0] << SL1) ^ (c[0] & MSK1) ^ (c[0] >> SR1)
	^ d[0] ^ (d[0] << SL3);
    a[1] = a[1] ^ x[1] ^ y[1] ^ (b[1] << SL1) ^ (c[1] & MSK1) ^ (c[1] >> SR1)
	^ d[1] ^ (d[1] << SL3);
    a[2] = a[2] ^ x[2] ^ y[2] ^ (b[2] << SL1) ^ (c[2] & MSK1) ^ (c[2] >> SR1)
	^ d[2] ^ (d[2] << SL3);
    a[3] = a[3] ^ x[3] ^ y[3] ^ (b[3] << SL1) ^ (c[3] & MSK1) ^ (c[3] >> SR1)
	^ d[3] ^ (d[3] << SL3);
}
