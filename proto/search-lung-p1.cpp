...
void check_init_lung(in_status base[], int size) {
    static char* initial[] = {
	"Mersenne Twister", "MERSENNE TWISTER",	"mersenne twister",
	"SIMDorientedFast", "SIMDorientedFAST",
	NULL};
    mat_GF2 matrix;
    int i, j, k;
    int rank;
	...
	for (j = 0; j < 4; j++) {
	    for (k = 0; k < 4; k++) {
		///¤È¤Á¤å¤¦
		lung[j] |= initial[i][j * 4 + k] << ((3 - k) * 8);
	    }
	}
	for (j = 0; j < 128; j++) {
	    matrix.put(size, j, initial[i][j / 16] (31 - j)) & 1);
	}
	rank = gauss(matrix);
	if (rank == size + 1) {
    ...
    printf("initial lung can't be found\n");
}
