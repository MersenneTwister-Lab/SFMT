...
void setup_param(unsigned int p1, unsigned int p2, unsigned int p3, 
    ...
    SL1 = p3 % (32 - 1) + 1;
    //SL1 = 5;
    MSK1= (1U << (p4 % 33)) ^ (0xffffffffU);
    MSK2= (1U << (p5 % 33)) ^ (0xffffffffU);
    MSK3= (1U << (p6 % 33)) ^ (0xffffffffU);
    MSK4= (1U << (p7 % 33)) ^ (0xffffffffU);
    //SL7 = p8 % (32 - 1) + 1;
    //SL8 = p9 % (32 - 1) + 1;
    //SR1 = p10 % (32 - 1) + 1;
    //SR2 = p11 % (32 - 1) + 1;
...
static inline void do_recursion(uint32_t a[4], uint32_t b[4], uint32_t c[4]) {
    ...
    t = ((uint64_t)b[0] << 32) | ((uint64_t)b[1]);
    t = t << SL1;
    bb[0] = (uint32_t)(t >> 32);
    bb[1] = (uint32_t)t;
    t = ((uint64_t)b[2] << 32) | ((uint64_t)b[3]);
    t = t << SL1;
    bb[2] = (uint32_t)(t >> 32);
    bb[3] = (uint32_t)t;
    tmp = a[3];
    a[3] = a[2] ^ bb[3] ^ (c[3] & MSK4);
    a[2] = a[1] ^ bb[2] ^ (c[2] & MSK3);
    a[1] = a[0] ^ bb[1] ^ (c[1] & MSK2);
    a[0] = tmp  ^ bb[0] ^ (c[0] & MSK1);
}
