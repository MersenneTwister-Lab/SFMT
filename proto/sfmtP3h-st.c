...
static unsigned int PM1 = 11;
static unsigned int PM2 = 11;
static unsigned int PM3 = 11;
static unsigned int PM4 = 11;
...
static inline void do_recursion(uint32_t a[4], uint32_t b[4],
    ...
    lshift128(x, a, SL2);
    a[0] = a[0] ^ x[0] ^ ((b[0] >> SR1) & MSK1) ^ c[0] ^ (c[0] << SL1) ^ d[PM1];
    a[1] = a[1] ^ x[1] ^ ((b[1] >> SR1) & MSK2) ^ c[1] ^ (c[1] << SL1) ^ d[PM2];
    a[2] = a[2] ^ x[2] ^ ((b[2] >> SR1) & MSK3) ^ c[2] ^ (c[2] << SL1) ^ d[PM3];
    a[3] = a[3] ^ x[3] ^ ((b[3] >> SR1) & MSK4) ^ c[3] ^ (c[3] << SL1) ^ d[PM4];
}
...
void get_lung(sfmt_t *sfmt, uint32_t lung[4]) {
    lung[0] = sfmt->sfmt[N][0];
    lung[1] = sfmt->sfmt[N][1];
    lung[2] = sfmt->sfmt[N][2];
    lung[3] = sfmt->sfmt[N][3];
}
...
void read_random_param(FILE *f) {
    ...
    fgets(line, 256, f);
    PM1 = get_uint(line, 10);
    fgets(line, 256, f);
    PM2 = get_uint(line, 10);
    fgets(line, 256, f);
    PM3 = get_uint(line, 10);
    fgets(line, 256, f);
    PM4 = get_uint(line, 10);
    fgets(line, 256, f);
