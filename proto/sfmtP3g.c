...
static inline void do_recursion(uint32_t a[4], uint32_t b[4],
    ...
    lshift128(x, d, SL2);
    a[0] = d[0] ^ x[0] ^ ((b[0] >> SR1) & MSK1) ^ (c[0] << SL1) ^ a[PM1];
    a[1] = d[1] ^ x[1] ^ ((b[1] >> SR1) & MSK2) ^ (c[1] << SL1) ^ a[PM2];
    a[2] = d[2] ^ x[2] ^ ((b[2] >> SR1) & MSK3) ^ (c[2] << SL1) ^ a[PM3];
    a[3] = d[3] ^ x[3] ^ ((b[3] >> SR1) & MSK4) ^ (c[3] << SL1) ^ a[PM4];
}
