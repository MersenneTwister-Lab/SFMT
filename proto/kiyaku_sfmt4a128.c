...
static void gen_rand_all(void) {
    ...
    for (i = 0; i < N; i++) {
	sfmt[i][0] = (sfmt[i][0] << SR1) ^ sfmt[i][1]
	    ^ (sfmt[(i + POS1) % N][0] >> SR1) ^ sfmt[(i + POS1) % N][1]
	    ^ (sfmt[(i + N - 1) % N][0] << SR1) ^ sfmt[(i + N -1) % N][3]; 
	sfmt[i][1] = (sfmt[i][1] << SL1) ^ sfmt[i][0]
	    ^ (sfmt[(i + POS1) % N][1] >> SL1) ^ sfmt[(i + POS1) % N][2]
	    ^ (sfmt[(i + N - 1) % N][1] << SL1) ^ sfmt[(i + N -1) % N][0];
	sfmt[i][2] = (sfmt[i][2] << SR2) ^ sfmt[i][3]
	    ^ (sfmt[(i + POS1) % N][2] >> SR2) ^ sfmt[(i + POS1) % N][3]
	    ^ (sfmt[(i + N - 1) % N][2] << SR2) ^ sfmt[(i + N -1) % N][1];
	sfmt[i][3] = (sfmt[i][3] << SL2) ^ sfmt[i][2]
	    ^ (sfmt[(i + POS1) % N][3] >> SL2) ^ sfmt[(i + POS1) % N][0]
