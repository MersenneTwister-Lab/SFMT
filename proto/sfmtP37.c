...
static inline __attribute__((always_inline)) 
    void do_recursion(uint32_t a[4], uint32_t b[4],
...
static inline void gen_rand_all(void) {
    ...
    do_recursion(sfmt[0], sfmt[POS1], sfmt[N - 1],
		 lung1);
    xor128(lung1, sfmt[0]);
    for (i = 1; i + POS1 < N; i++) {
	do_recursion(sfmt[i], sfmt[i + POS1], sfmt[i - 1],
		     lung1);
	xor128(lung1, sfmt[i]);
    }
    for (; i < N; i++) {
	do_recursion(sfmt[i], sfmt[i + POS1 - N], sfmt[i - 1],
		     lung1);
	xor128(lung1, sfmt[i]);
    }
