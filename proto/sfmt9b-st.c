...
void print_param2(FILE *fp) {
    fprintf(fp, "PARAMS[POS1, SL1, SL2, SR1, SR2] = [%u,%u,%u,%u,%u,%u]\n", 
	    POS1, SL1, SL2, SL3,SR1, SR2);
    fflush(fp);
}
...
void next_state(sfmt9_t *sfmt) {
	...
	//^ (sfmt->sfmt[sfmt->idx] >> SR1)
	^ sfmt->sfmt[sfmt->idx]
	^ (sfmt->sfmt[(sfmt->idx + POS1) % N] << SL2)
	^ sfmt->sfmt[(sfmt->idx + POS1) % N]
	^ (sfmt->sfmt[(sfmt->idx + N - 4) % N] << SL3) 
	^ (sfmt->sfmt[(sfmt->idx + N - 4) % N] >> SR2);
}
