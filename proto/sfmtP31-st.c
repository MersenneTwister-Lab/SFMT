...
static inline void do_recursion(uint32_t a[4], uint32_t b[4],
    ...
    lshift128(x, a, SL2);
    rshift128(y, c, SR2);
    a[0] = a[0] ^ x[0] ^ (b[0] >> SR1) ^ (b[0] & MSK1) ^ y[0] ^ (d[0] << SL1);
    a[1] = a[1] ^ x[1] ^ (b[1] >> SR1) ^ (b[1] & MSK2) ^ y[1] ^ (d[1] << SL1);
    a[2] = a[2] ^ x[2] ^ (b[2] >> SR1) ^ (b[2] & MSK3) ^ y[2] ^ (d[2] << SL1);
    a[3] = a[3] ^ x[3] ^ (b[3] >> SR1) ^ (b[3] & MSK4) ^ y[3] ^ (d[3] << SL1);
}
