...
static unsigned int POS1 = 89;
static unsigned int SL1 = 18;
static unsigned int SL2 = 1;
static unsigned int SL3 = 10;
static unsigned int SR1 = 3;
static unsigned int SR2 = 2;
static uint32_t MSK1 = 0x7f7fffffUL;
static uint32_t MSK2 = 0xfffbffffUL;
static uint32_t MSK3 = 0xeffffbffUL;
static uint32_t MSK4 = 0xfefe7befUL;
...
static inline void perm(uint32_t to[4], uint32_t from[4], int type) {
    to[0] ^= from[3];
    to[1] ^= from[2];
    to[2] ^= from[0];
    to[3] ^= from[1];
...
static inline void do_recursion(uint32_t a[4], uint32_t b[4],
    ...
    lshift128(x, a, SL2);
    a[0] = a[0] ^ x[0] ^ ((b[0] >> SR1) & MSK1) ^ (c[0] >> SR2) ^ (c[0] << SL1) ^ d[3];
    a[1] = a[1] ^ x[1] ^ ((b[1] >> SR1) & MSK2) ^ (c[1] >> SR2) ^ (c[1] << SL1) ^ d[2];
    a[2] = a[2] ^ x[2] ^ ((b[2] >> SR1) & MSK3) ^ (c[2] >> SR2) ^ (c[2] << SL1) ^ d[0];
    a[3] = a[3] ^ x[3] ^ ((b[3] >> SR1) & MSK4) ^ (c[3] >> SR2) ^ (c[3] << SL1) ^ d[1];
}
